...
% procedure ("Model_Mapping_code.m") where the real-time growth rate r is
% matched between epidemics rather than R0. Because there is no machine
...
% desired_r =  0.14552; R0 = 1.5; % The value of R0 is used only for cross-checking
desired_r =  0.25282; R0 = 2; % The value of R0 is used only for cross-checking
% desired_r =  0.52588; R0 = 4; % The value of R0 is used only for cross-checking
...
workspace_name = [ 'r', num2str(round(desired_r*100000),'%06d'),...
...
workspace_name = [ workspace_name, '_rfixed_nsimr',num2str(nsimr,'%05d') ];
...
%                 end
                ...
                if Activate_checks
                    disp([num2str(nsimr),' simulations lead to |R0-R1| = ',num2str(abs(R0-R1))]); % Check the computed R0 is what it should be
...
% zlim([0,1]);
zlim(afs_limits);
title('afs AH model matching r','Fontsize',16);
