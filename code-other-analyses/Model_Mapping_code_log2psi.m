...
% (this script) is saved in a workspace with the following name:
workspace_name = [ 'R0', num2str(R0*10,'%02d'),...
    '_pAA', num2str(ceil(pAA_min*100),'%02d'),'_',num2str(ceil(dpAA*100),'%02d'),'_',num2str(round(pAA_max*100),'%02d'),...
...
%                 disp(['Computation of Rg from r for loop ',num2str(i2),'-',num2str(i1),' of ',num2str(l2),'-',num2str(l1)]);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%% A model %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if ( theta_A(i1,i2) == mapfail ) || ( isnan(mapfail) && isnan(theta_A(i1,i2)) )
                    Rg_A(i1,i2) = mapfail;
                    NGM_A = mapfail;
