...
D.Y = y_vec;
D.Z = relreg_list;
D.nZover = 2; % Two different levels of overlaid information
D.Zover = { overlaid_list, overlaid_list };
D.Zcont = { [ 10 30 50 70 90 ], [ 20 40 60 80 ] };
D.Zshowtext = { 'off', 'on' };
D.Zlinewidth = { 0.3, 0.6 };
D.Clim = [ -4.5, 4.5 ];
D.Climleg = [ -0.5, 4.5 ];
...
if Activate_save_fig
    ...
    cd(fig_fig_path);
    export_fig Main_Figure3right -pdf -nocrop -transparent;
    cd(fig_code_path);
